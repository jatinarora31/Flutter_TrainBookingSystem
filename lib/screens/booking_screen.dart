...
class _BookingScreenState extends State<BookingScreen>
  ...
  double _fareForCoachType(FareOptions fare, String? coachType) {
    switch (coachType) {
      case 'one_ac':
        return fare.oneAc?.farePerSeat ?? 0;
      case 'two_ac':
        return fare.twoAc?.farePerSeat ?? 0;
      case 'sleeper':
        return fare.sleeper?.farePerSeat ?? 0;
      default:
        return 0;
    }
  }
        ...
        _BottomBar(
          ...
          onBook: _selectedSeatIds.isNotEmpty
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                    '${_selectedSeatIds.length} seat(s) selected. Proceed to payment!'),
...
class _CoachTab extends StatelessWidget {
  ...
  double _fareForType(String coachType) {
    switch (coachType) {
      case 'one_ac':
        return fareOptions.oneAc?.farePerSeat ?? 0;
      case 'two_ac':
        return fareOptions.twoAc?.farePerSeat ?? 0;
      case 'sleeper':
        return fareOptions.sleeper?.farePerSeat ?? 0;
      default:
        return 0;
    }
  }
